...
const PROFILES = {
  profiles: [
    {
      profileId: 999,
      displayName: 'Mikkel Eksempelsen',
      institutionProfiles: [
...
const PROFILE_CONTEXT = {
  ...
  institutionProfile: { fullName: 'Mikkel Eksempelsen' },
  institutions: [
    {
      institutionCode: '100001',
      institutionName: 'Eksempelskolen',
...
const MESSAGES: Record<number, { from: string; role: string; ago: number; html: string }[]> = {
  5001: [
    { from: 'Lone Lærke', role: 'employee', ago: -4, html: 'Vi tager af sted mandag den 25. Husk regntøj.' },
    { from: 'Far Eksempelsen', role: 'guardian', ago: -3, html: 'Skal de have madpakke med begge dage?' },
    { from: 'Lone Lærke', role: 'employee', ago: -2, html: 'Kun mandag. Tirsdag sørger vi for maden.' },
  ...
  switch (method) {
    ...
    case 'gallery.getAlbums': {
      ...
      const visible = ALBUMS.filter((a) => ids.has(a.forId)).map((a) => ({
        ...
        creator: { name: 'Lone Lærke', institutionName: a.inst },
