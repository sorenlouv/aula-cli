 ...
 * (Peters far)* floored every message from a teacher called Peter. Prose is the
 ...
 * represent with one entry, and the answer is no across the hidden boundary.
 * Absorbing a hidden signal into a visible one shows what the family asked to
 * put away; absorbing a visible one into a hidden one takes it off the page
 * altogether — into no card, no "Godt at vide" and no muted foot, since
 * `mergedSourceKeys` counts as covered everywhere downstream. Same subject,
 * same date, different answer to "do you want this", so: two entries.
 */
function interchangeable(a: RankedSignal, b: RankedSignal): boolean {
  return (a.tier === 'hidden') === (b.tier === 'hidden');
}
...
export function rank(
  ...
  // another, so a key can hold a hidden entry and a visible one.
