...
describe("the family's list, as the model read it", () => {
  ...
  // floored a teacher called Peter on a wish about Peters far.
  ...
  const johnsThread = (key = 'thread:7') =>
    item({ key, kind: 'thread', title: 'Legeaftale', author: 'John Madsen', audience: 'institution' });
  ...
  test('a high source no signal covered still reaches the page', () => {
    ...
    const items = [item({ key: 'thread:8', kind: 'thread', title: 'Hej igen', text: 'Vi ses på fredag.', author: 'John Madsen' })];
  ...
  test('high outranks the audience prior', () => {
    ...
    const items = [item({ key: 'post:10', title: 'Fra forvaltningen', author: 'John Madsen', audience: 'municipal' })];
  ...
  test('the verdict moves the score, so high survives the cap', () => {
    ...
    const items = Array.from({ length: ACT_CAP + 1 }, (_, i) =>
      item({ key: `plan:x:${i}`, title: `Opgave ${i}`, text: `Husk ${i} madpakke.`, childNames: ['Viggo Emil Eksempelsen'], audience: 'child' }),
    );
