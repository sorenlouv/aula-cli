...
test('remember, preferences, forget — the curation round trip', () => {
  ...
  const remembered = box.run('remember', 'beskeder fra John (Peters far) er altid vigtige');
  assert.equal(remembered.code, 0, remembered.stderr);
  assert.match(remembered.stdout, /Remembered:/);
  assert.match(readFileSync(path, 'utf8'), /^- beskeder fra John \(Peters far\) er altid vigtige$/m);
  assert.match(box.run('preferences').stdout, /6\. beskeder fra John/);

  // Claude will say it twice sooner or later; twice is still once.
  assert.match(box.run('remember', 'Beskeder fra John (Peters far) er ALTID vigtige').stdout, /Already remembered/);
...
test('a remembered wish reaches the model that writes the overview', () => {
  ...
  box.run('remember', 'beskeder fra John (Peters far) er altid vigtige');
  ...
  assert.match(calls, /beskeder fra John \(Peters far\) er altid vigtige/);
...
test('preferences reset puts the shipped list back and names the casualties', () => {
  ...
  assert.match(reset.stdout, /beskeder fra John \(Peters far\)/);
